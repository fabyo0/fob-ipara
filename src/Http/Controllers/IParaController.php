...
use Botble\Payment\Facades\PaymentHelper;
            ...
            if (isset($data['orderId']) && isset($data['result'])) {
                ...
                if ($result === 'success' || $result === '1') {
                    ...
                    if ($payment) {
                        ...
                        do_action(PAYMENT_ACTION_PAYMENT_CONFIRMED, $payment);
    ...
    public function process(Request $request)
    {
        ...
        try {
            ...
            if (is_plugin_active('ecommerce')) {
                $order = \Botble\Ecommerce\Models\Order::with(['user', 'address', 'shippingAddress'])->find($payment->order_id);
                if ($order) {
                    $customer = $order->user;
                }
            } elseif (is_plugin_active('hotel')) {
                $booking = \Botble\Hotel\Models\Booking::with(['customer', 'address'])->find($payment->order_id);
                if ($booking) {
